...
dygraph.func(model = fc.reg, main = "Linear Regression Model")
```
...
__<span style="background-color:#FFE1FF;">Comment</span>__: Figure 2 plots the actual (training) data versus the fitted data. If the predictions are close to the actual values,$\text{R}^2$ is expected to be close to 1. The Pearson correlation test shows that the correlation between these variables is  $\text{R}^2$ = 0.3225668. In this case the model doesn't seem like an appropriate choice for fitting the data of Family A as it only explains 32% of the variation in the data. 
